...
from Fnc_Stk_Fts_2D import *
from Fnc_Stk_Mth_2D import *

from Fnc_Stk_Plt_2D import align_yaxis, ScaledFormatter
...
def fit_2D_Gaussian(Cube2bFit,*args,**kwargs):
	...
	if displ_s_f == True:
		...
		#ax110.xaxis.set_major_locator(ScaledLocator(dx=1.0,x0=-X0_f2DG + x_ref )) #X_DIF
		#ax110.yaxis.set_major_locator(ScaledLocator(dx=1.0,x0=-Y0_f2DG + y_ref )) #Y_DIF
...
def fit_1D_Gaussian(Cube2bPlot_1D,*args, **kwargs):
	...
	label_SUM = 'SUM' #str(Cube2bPlot_1D.rsplit('/',1)[1]) + ':  ' + str(id_ref)+ '  ' + str(z_ref)+ '  ' + str(id_zro)+ '  ' + str(z_zro)  
	label_AVG = 'AVG'
	label_MED = 'MED'
	label_STD = 'STD'
	...
	f = plt.figure(num=None, figsize=(11, 8), dpi=180, facecolor='w',
		edgecolor='k')
	plt.subplots_adjust(
		left 	= (38/25.4)/fxsize,    #18
		bottom 	= (14/25.4)/fysize, 
		right 	= 1 - (22/25.4)/fxsize, #6
		top 	= 1 - (15/25.4)/fysize)
	plt.subplots_adjust(hspace=0)
	...
	try:
		...
		#Luminosity_Error(lumi,rdshft_inf,rdshft_sup,flux_error,*args, **kwargs)
		XAXIS_FIT     = np.arange(XAXIS[0],XAXIS[-1],0.01)
		ax210_twin.plot(XAXIS_FIT, g_sum(XAXIS_FIT), color='green',ls='--',alpha=0.4,
			label='Gaussian fit ('+ label_SUM +
					...
					', $\sigma$ : '+ str(np.round(g_sum.stddev[0],3))                 + ' $\pm$ ' + str(np.round(g_sum_var_std,3)))
...
def CCube_Spatial_Extract_Circular(CSEC_ifn,X_C,Y_C,radii_px_inner,radii_as_inner,radii_px_outer,radii_as_outer,radii_px_msure,radii_as_msure,*args,**kwargs):
	dest_dir_stp  = kwargs.get('dest_dir_stp',None)
	verbose   = kwargs.get('verbose',None)
	plt_slcs  = kwargs.get('plt_slcs',False)
	...
	Splt_Hdr_Cmt_cp = kwargs.get('Splt_Hdr_Cmt_cp_cp',None)
	...
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_AVG',header_comment='Line Width [GHz] Average')               for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_MED',header_comment='Line Width [GHz] Median')                for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_1SL',header_comment='Line Width [GHz] 1 sgm lw lmt 15.9 pct') for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_1SH',header_comment='Line Width [GHz] 1 sgm hg lmt 84.1 pct') for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_2SL',header_comment='Line Width [GHz] 2 sgm lw lmt 2.30 pct') for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_2SH',header_comment='Line Width [GHz] 2 sgm hg lmt 97.7 pct') for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_3SL',header_comment='Line Width [GHz] 3 sgm lw lmt 0.20 pct') for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_3SH',header_comment='Line Width [GHz] 3 sgm hg lmt 99.8 pct') for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_P25',header_comment='Line Width [GHz] 25 pct')                for csec_ofns_cube in csec_ofns]
	[Header_Copy(csec_ofns_cube,CSEC_ifn,'STW_P75',header_comment='Line Width [GHz] 75 pct')                for csec_ofns_cube in csec_ofns]
...
def CCube_fit_2D_Gaussian(Cube2bFit,*args,**kwargs):
	...
	if dest_dir_plt != None and os.path.exists(dest_dir_plt) == False:
		print ('Directory does not exist, creating it: ', dest_dir_plt)
		os.makedirs(dest_dir_plt)
	else:
		pass
	if dest_dir_clp != None and os.path.exists(dest_dir_clp) == False:
		print ('Directory does not exist, creating it: ', dest_dir_plt)
		os.makedirs(dest_dir_clp)
	else:
		pass
	if slc_nmb == None:
		if sgm_fnc == 'avg':
			slice_nmbr   = (Header_Get(Cube2bFit,'MAX_SNA'))
			slice_sigm   = (Header_Get(Cube2bFit,'FTA_SIG'))
			slice_fwhm   = (Header_Get(Cube2bFit,'FTA_FWH'))
			slice_cct    = (Header_Get(Cube2bFit,'FTA_CCT'))
			slice_1sg    = (Header_Get(Cube2bFit,'FTA_1SG'))
			slice_2sg    = (Header_Get(Cube2bFit,'FTA_2SG'))
			slice_3sg    = (Header_Get(Cube2bFit,'FTA_3SG'))
			slice_4sg    = (Header_Get(Cube2bFit,'FTA_4SG'))
			slice_5sg    = (Header_Get(Cube2bFit,'FTA_5SG'))
			slice_1fw    = (Header_Get(Cube2bFit,'FTA_1FW'))
			slice_2fw    = (Header_Get(Cube2bFit,'FTA_2FW'))
		elif sgm_fnc == 'med':
			slice_nmbr   = (Header_Get(Cube2bFit,'MAX_SNM'))
			slice_sigm   = (Header_Get(Cube2bFit,'FTM_SIG'))
			slice_fwhm   = (Header_Get(Cube2bFit,'FTM_FWH'))
			slice_cct    = (Header_Get(Cube2bFit,'FTM_CCT'))
			slice_1sg    = (Header_Get(Cube2bFit,'FTM_1SG'))
			slice_2sg    = (Header_Get(Cube2bFit,'FTM_2SG'))
			slice_3sg    = (Header_Get(Cube2bFit,'FTM_3SG'))
			slice_4sg    = (Header_Get(Cube2bFit,'FTM_4SG'))
			slice_5sg    = (Header_Get(Cube2bFit,'FTM_5SG'))
			slice_1fw    = (Header_Get(Cube2bFit,'FTM_1FW'))
			slice_2fw    = (Header_Get(Cube2bFit,'FTM_2FW'))
		elif sgm_fnc == 'sum':
			slice_nmbr   = (Header_Get(Cube2bFit,'MAX_SNS'))
			slice_sigm   = (Header_Get(Cube2bFit,'FTS_SIG'))
			slice_fwhm   = (Header_Get(Cube2bFit,'FTS_FWH'))
			slice_cct    = (Header_Get(Cube2bFit,'FTS_CCT'))
			slice_1sg    = (Header_Get(Cube2bFit,'FTS_1SG'))
			slice_2sg    = (Header_Get(Cube2bFit,'FTS_2SG'))
			slice_3sg    = (Header_Get(Cube2bFit,'FTS_3SG'))
			slice_4sg    = (Header_Get(Cube2bFit,'FTS_4SG'))
			slice_5sg    = (Header_Get(Cube2bFit,'FTS_5SG'))
			slice_1fw    = (Header_Get(Cube2bFit,'FTS_1FW'))
			slice_2fw    = (Header_Get(Cube2bFit,'FTS_2FW'))
		elif sgm_fnc == None:
			print
			print colored('Using average fwhm line width from gaussian fit.','yellow')
			print colored('From fits header STW_AVG','yellow')
			print Cube2bFit
			print
			slice_fwhm   = (Header_Get(Cube2bFit,'STW_AVG'))
			slice_1sg    = fwhm2sigma(slice_fwhm)
			slice_2sg    = slice_1sg*2
			slice_3sg    = slice_1sg*3
			slice_4sg    = slice_1sg*4
			slice_5sg    = slice_1sg*5
			slice_1fw    = slice_fwhm
			slice_2fw    = slice_fwhm*2
			pass
	...
	cube_data    = np.asarray(apfts.getdata(Cube2bFit,memmap=False))
	...
	if slc_nmb == None:
		if sgm_wgth_tms == '1sgm':
			...
			#Since the fwhm comes from measurments in CASA
			#transformed into channels to collapse the cube
			#through immoments, or get the average through
			#split it already includes the total DV = 1Xfwhm
			#No need to include it here in the total vel width cl_wdt_clp
			print
			...
			print
			slice_sigm = fwhm2sigma(slice_fwhm)
			vl_wdt_clp = slice_fwhm
		elif sgm_wgth_tms == 'slice_2fw':
			#Since the fwhm comes from measurments in CASA
			#transformed into channels to collapse the cube
			#through immoments, or get the average through
			#split it already includes the total DV = 2Xfwhm
			#No need to include it here in the total vel width cl_wdt_clp
			print
			print colored('Velocity width interval (fwhm) : ' + str(sgm_wgth_tms),'yellow')
			print colored('Cube fitting 2Xsigm (avg) km/s : ' + str(fwhm2sigma(slice_fwhm)),'yellow')
			print colored('Cube fitting 2Xfwhm (avg) km/s : ' + str(slice_fwhm),'yellow')
			print
			slice_sigm = fwhm2sigma(slice_fwhm)
			vl_wdt_clp = slice_fwhm
	elif slc_nmb != None:
	...
	if slc_nmb != None:
		data_2b_plot = cube_data[slc_nmb]
		Message = 'Fitting gaussian with slice number : ' + str(slc_nmb+1)
		plt_tlt = 'Slice: '
		clp_hdr = 'C'  
		clp_hdc = 'CSL' 
		data_2b_plt = np.asarray(apfts.getdata(Cube2bFit,memmap=False))
		data_2b_plt_clp = data_2b_plt[slc_nmb]
		data_2b_plot    = data_2b_plt_clp
	elif slc_nmb == None:
		Message = 'Fitting gaussian through cube collapse ('+str(clp_fnc)+')'
		plt_tlt = '2D Collapse '
		data_2b_plt = np.asarray(apfts.getdata(Cube2bFit,memmap=False))
		data_2b_plot = cube_data[0]
		clp_hdr = 'C' 
		clp_hdc = 'CSL' 
	...
	try:
		...
		f,nx_f2DG, ny_f2DG = data_2b_plot.shape
	nx,ny            = nx_f2DG,ny_f2DG
	...
	#CHANGE LIMITS FOR GAUSSIAN SIZE IF FIT FAILS 1-->2#	
	try:
		if fit_type == 'scipy':
			print
		...
		data_fitted = func_2D_Gaussian((x, y), *popt, circular=circular)
		fit_res     = 'ERR'
		print colored('Error - curve_fit failed','yellow')
		print RuntimeError
		print
	...
	if sgm_fnc == 'avg':
		...
		print float(sigma_fwhm_2d)
	g_obs = data_2b_plot
	g_exp = func_2D_Gaussian((x, y),X0_f2DG,Y0_f2DG,1,sigma_fwhm_2d,sigma_fwhm_2d,THETA_F,OFFSET_F,circular=circular)

	if Cube2bFit_Err == None:

		g_obs_sgm = g_obs
		g_exp_sgm = func_2D_Gaussian((x, y),X0_f2DG,Y0_f2DG,1,SIGMAX_F,SIGMAY_F,THETA_F,OFFSET_F,circular=circular)
		g_obs_amp = g_obs
		...
		print
		cube_data_err    = np.asarray(apfts.getdata(Cube2bFit_Err,memmap=False) )
		g_err = cube_data_err
	...
	print
	DGF_vlm = abs((SIGMAX_F/SIGMAX_f2DG)) * abs((SIGMAX_F/SIGMAX_f2DG)) * A_F #* 2 * np.pi 
	DGF_vx1 = abs((SIGMAX_F/SIGMAX_f2DG)) * abs((SIGMAX_F/SIGMAX_f2DG)) * A_F * slice_sigm#* 2 * np.pi  * slice_sigm
	DGF_vx2 = abs((SIGMAX_F/SIGMAX_f2DG)) * abs((SIGMAX_F/SIGMAX_f2DG)) * A_F * vl_wdt_clp#* 2 * np.pi  * vl_wdt_clp
	DGF_vx3 = abs((SIGMAX_F/SIGMAX_f2DG)) * abs((SIGMAX_F/SIGMAX_f2DG)) * A_F #* 2 * np.pi  #* slice_cwdt * tlt_ch_nmb
	if src_sze_fxd == True:
		...
		DGF_v3e = (((SIGMAX_F/SIGMAX_f2DG)**1)) * (np.sqrt((((SIGMAX_F/SIGMAX_f2DG)**2) * (A_E**2))))# + (4 * (A_F**2) * (SIGMAX_E**2)))) #* slice_cwdt * tlt_ch_nmb
	...
	BSF_vx2 = brt_flx_sum  * vl_wdt_clp
	BSF_vx3 = brt_flx_sum 

	BSF_vle = brt_flx_std
	BSF_v1e = brt_flx_std * slice_sigm
	BSF_v2e = brt_flx_std * vl_wdt_clp
	BSF_v3e = brt_flx_std 
	...
	[Header_Add(CUBE,clp_hdr + '2G_FT2',DGF_vx2   ,header_comment = '2DGF Vol X 1DNXFWHM ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2G_FT3',DGF_vx3   ,header_comment = '2DGF Vol ' + clp_hdc) for CUBE in CUBES_2DGF]
	...
	[Header_Add(CUBE,clp_hdr + '2G_F2E',DGF_v2e   ,header_comment = '2DGF Vol X 1DNXFWHM ' + clp_hdc + 'Err') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2G_F3E',DGF_v3e   ,header_comment = '2DGF Vol' + clp_hdc + 'Err') for CUBE in CUBES_2DGF]
	...
	[Header_Add(CUBE,clp_hdr + '2G_LM2',DGF_v2l[0],header_comment = '2DGF Vol X 1DNXFWHM Lum ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2G_LL2',DGF_v2l[1],header_comment = '2DGF Vol X 1DNXFWHM Lum [log] ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2G_LM3',DGF_v3l[0],header_comment = '2DGF VolLum ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2G_LL3',DGF_v3l[1],header_comment = '2DGF VolLum [log] ' + clp_hdc) for CUBE in CUBES_2DGF]
	...
	[Header_Add(CUBE,clp_hdr + 'BS_FT2',BSF_vx2   ,header_comment = 'BSF Vol X 1DNXFWHM ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BS_FT3',BSF_vx3   ,header_comment = 'BSF Vol' + clp_hdc) for CUBE in CUBES_2DGF]
	...
	[Header_Add(CUBE,clp_hdr + 'BS_F2E',BSF_v2e   ,header_comment = 'BSF Vol X 1DNXFWHM ' + clp_hdc + 'Err') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BS_F3E',BSF_v3e   ,header_comment = 'BSF Vol' + clp_hdc + 'Err') for CUBE in CUBES_2DGF]
	...
	[Header_Add(CUBE,clp_hdr + 'BS_LM2',BSF_v2l[0],header_comment = 'BSF Vol X 1DNXFWHM Lum ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BS_LL2',BSF_v2l[1],header_comment = 'BSF Vol X 1DNXFWHM Lum [log] ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BS_LM3',BSF_v3l[0],header_comment = 'BSF VolLum ' + clp_hdc) for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BS_LL3',BSF_v3l[1],header_comment = 'BSF VolLum [log] ' + clp_hdc) for CUBE in CUBES_2DGF]
	...
	[Header_Add(CUBE,clp_hdr + '2L_21L',lum_area_err_1_2[0], header_comment = '2DGF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_21H',lum_area_err_1_2[1], header_comment = '2DGF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_21L',lum_area_err_1_2[2], header_comment = '2DGF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_21H',lum_area_err_1_2[3], header_comment = '2DGF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_22L',lum_area_err_2_2[0], header_comment = '2DGF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_22H',lum_area_err_2_2[1], header_comment = '2DGF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_22L',lum_area_err_2_2[2], header_comment = '2DGF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_22H',lum_area_err_2_2[3], header_comment = '2DGF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_23L',lum_area_err_3_2[0], header_comment = '2DGF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_23H',lum_area_err_3_2[1], header_comment = '2DGF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_23L',lum_area_err_3_2[2], header_comment = '2DGF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_23H',lum_area_err_3_2[3], header_comment = '2DGF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	
	[Header_Add(CUBE,clp_hdr + '2L_31L',lum_area_err_1_3[0], header_comment = '2DGF Vol Lum T' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_31H',lum_area_err_1_3[1], header_comment = '2DGF Vol Lum T' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_31L',lum_area_err_1_3[2], header_comment = '2DGF Vol Lum T[log] ' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_31H',lum_area_err_1_3[3], header_comment = '2DGF Vol Lum T[log] ' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_32L',lum_area_err_2_3[0], header_comment = '2DGF Vol Lum T' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_32H',lum_area_err_2_3[1], header_comment = '2DGF Vol Lum T' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_32L',lum_area_err_2_3[2], header_comment = '2DGF Vol Lum T[log] ' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_32H',lum_area_err_2_3[3], header_comment = '2DGF Vol Lum T[log] ' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_33L',lum_area_err_3_3[0], header_comment = '2DGF Vol Lum T' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2L_33H',lum_area_err_3_3[1], header_comment = '2DGF Vol Lum T' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_33L',lum_area_err_3_3[2], header_comment = '2DGF Vol Lum T[log] ' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + '2M_33H',lum_area_err_3_3[3], header_comment = '2DGF Vol Lum T[log] ' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	...
	[Header_Add(CUBE,clp_hdr + 'BL_21L',lum_area_brt_err_1_2[0], header_comment = 'BSF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_21H',lum_area_brt_err_1_2[1], header_comment = 'BSF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_21L',lum_area_brt_err_1_2[2], header_comment = 'BSF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_21H',lum_area_brt_err_1_2[3], header_comment = 'BSF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_22L',lum_area_brt_err_2_2[0], header_comment = 'BSF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_22H',lum_area_brt_err_2_2[1], header_comment = 'BSF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_22L',lum_area_brt_err_2_2[2], header_comment = 'BSF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_22H',lum_area_brt_err_2_2[3], header_comment = 'BSF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_23L',lum_area_brt_err_3_2[0], header_comment = 'BSF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_23H',lum_area_brt_err_3_2[1], header_comment = 'BSF Vol Lum T X 1DNXFWHM ' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_23L',lum_area_brt_err_3_2[2], header_comment = 'BSF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_23H',lum_area_brt_err_3_2[3], header_comment = 'BSF Vol Lum T X 1DNXFWHM [log] ' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	
	[Header_Add(CUBE,clp_hdr + 'BL_31L',lum_area_brt_err_1_3[0], header_comment = 'BSF Vol Lum T' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_31H',lum_area_brt_err_1_3[1], header_comment = 'BSF Vol Lum T' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_31L',lum_area_brt_err_1_3[2], header_comment = 'BSF Vol Lum T[log] ' + clp_hdc +' Err 1 sgm lw lmt 15.9 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_31H',lum_area_brt_err_1_3[3], header_comment = 'BSF Vol Lum T[log] ' + clp_hdc +' Err 1 sgm hg lmt 84.1 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_32L',lum_area_brt_err_2_3[0], header_comment = 'BSF Vol Lum T' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_32H',lum_area_brt_err_2_3[1], header_comment = 'BSF Vol Lum T' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_32L',lum_area_brt_err_2_3[2], header_comment = 'BSF Vol Lum T[log] ' + clp_hdc +' Err 2 sgm lw lmt 2.30 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_32H',lum_area_brt_err_2_3[3], header_comment = 'BSF Vol Lum T[log] ' + clp_hdc +' Err 2 sgm hg lmt 97.7 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_33L',lum_area_brt_err_3_3[0], header_comment = 'BSF Vol Lum T' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BL_33H',lum_area_brt_err_3_3[1], header_comment = 'BSF Vol Lum T' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_33L',lum_area_brt_err_3_3[2], header_comment = 'BSF Vol Lum T[log] ' + clp_hdc +' Err 3 sgm lw lmt 0.20 pct') for CUBE in CUBES_2DGF]
	[Header_Add(CUBE,clp_hdr + 'BM_33H',lum_area_brt_err_3_3[3], header_comment = 'BSF Vol Lum T[log] ' + clp_hdc +' Err 3 sgm hg lmt 99.8 pct') for CUBE in CUBES_2DGF]
	...
	g_res_img     = (g_obs_img - g_mdl_img)
	g_res_img     = (np.asarray(g_res_img)).reshape(nx, ny)
	spec_file_sum     = Wrt_FITS_File(g_mdl_img,FITSFILENAME_MDL)
	spec_file_sum     = Wrt_FITS_File(g_res_img,FITSFILENAME_RSD)
	####################MODEL AND RESIDUAL####################
	if verbose == True:
		print
		print 'Initial Guess:'
		...
		print 'Shift from the Y coordinate center:',Y_DIF
		print
		print colored('Generated Plot Fit       : ' + str(PLOTFILENAME) ,'cyan')
	...
	if displ_s_f == True:
		...
		cbar_min = np.amin(g_obs_img)
		cbar_max = np.amax(g_obs_img)
		g_obs_img,g_mdl_img
		...
		cbar = plt.colorbar(cax=cax)
		cbar.set_label('S [Jy]', rotation=270)
		...
		f = plt.figure(num=None, figsize=(fxsize, fysize), dpi=180, facecolor='w',
			edgecolor='k')
		plt.subplots_adjust(
			left 	= (8/25.4)/fxsize,        #-26 bigger 22-def
			bottom 	= (2/25.4)/fysize,        # 20 bigger 19-def
		...
		try:
			...
			x_text_coor =   0  		#IN TERMS OF COLORBAR NEW AXIS CAX
			y_text_coor =   -5 		#IN TERMS OF COLORBAR NEW AXIS CAX
			if '-5as_' in Cube2bFit:
		...
		print min_y,max_y
		print 'Dif'		
		print max_x-(abs(min_x-max_x)*.20), max_x-(abs(min_x-max_x)*.10)
		print
...
def CCube_fit_2D_Gaussian_Noise(Cube2bFit,*args,**kwargs):
	slc_nmb  = kwargs.get('slc_nmb' ,None)
	dest_dir = kwargs.get('dest_dir',None)
	verbose  = kwargs.get('verbose' ,None)
	clp_fnc  = kwargs.get('clp_fnc' ,'sum')
		...
		if slice_nblw!=slice_nbhg:
			...
			for j in indxs_all:
				if j not in indxs_fbd and (len(indxs_clp)<tlt_ch_nmb):
					...
					break
			data_2b_plt_clp = data_2b_plt[indxs_clp]#data_2b_plt[slice_nblw:slice_nbhg]
			skp_clp = False
