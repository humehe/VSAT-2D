...
def Plot_LuminosityVS(Redshift_bin,Var2,*args, **kwargs):
    dest_dir       = kwargs.get('dest_dir' ,None)
    ...
    markers_plt    = kwargs.get('markers_plt',1)
    colors_plt     = kwargs.get('colors_plt' ,1)
    labels_plt     = kwargs.get('labels_plt' ,1)

    linectrnm      = kwargs.get('linectrnm',None)
    linectrfr      = kwargs.get('linectrfr',None)

    variable_x     = kwargs.get('variable_x','z')
    variable_y     = kwargs.get('variable_y','Luminosity')
    plt_ofn_sfx    = kwargs.get('plt_ofn_sfx','')
    plt_ofn_pfx    = kwargs.get('plt_ofn_pfx',variable_x)

    logscale       = kwargs.get('logscale',None)
    errorbars      = kwargs.get('errorbars',False)
    E_Z1           = kwargs.get('E_Z1',None)
    E_Z2           = kwargs.get('E_Z2',None)
    E_V1           = kwargs.get('E_V1',None)
    E_V2           = kwargs.get('E_V2',None)
    ...
    if line_ratio == False:
        ...
        line_lbl   = '12_13CO'
        line_lbl_a = 'S$_{^{12}CO}$/S$_{^{13}CO}$'
        PLOT_TITLE = line_lbl + variable_y
        LR         = 'R'
    elif line_ratio == True and variable_y == 'Luminosity':
        line_lbl   = '12_13CO'
        line_lbl_a = 'L$_{^{12}CO}$/L$_{^{13}CO}$'
        PLOT_TITLE = line_lbl + variable_y
        LR         = 'R'
    if dest_dir != None:
        PLOTFILENAME = str(dest_dir)  + '/' + plt_ofn_pfx + 'vs'+variable_y+'-'+ line_lbl + LR + plt_ofn_sfx +'.pdf'
    elif dest_dir == None:
        PLOTFILENAME = plt_dir_res    + '/' + plt_ofn_pfx + 'vs'+variable_y+'-'+ line_lbl + LR + plt_ofn_sfx +'.pdf'

    if plt_lit_res == True:
        print
        print (colored('Plotting Literature Results!','yellow'))
        print (colored('From table: '+plt_lit_tbl,'yellow'))
        print
        var_lit_res = Table_Read_Lit(plt_lit_tbl,tbl_format_ipt)
        x  = var_lit_res[2]
        y  = var_lit_res[3]
        ye = var_lit_res[4]
    elif plt_lit_res == False:
        pass
    ...
    for j in range(len(Var2)):
        Redshift_bin[j][0] = Redshift_bin[j][0] + 0.01
        if errorbars == False:
            plt.scatter(Redshift_bin[j][1:], Var2[j][1:],color=colors_plt[j]     ,marker=markers_plt[j], s=14,label=labels_plt[j])
            plt.scatter(Redshift_bin[j][0] , Var2[j][0] ,color=colors_plt[j]     ,marker=markers_plt[j], s=14)
            plt.scatter(Redshift_bin[j][0] , Var2[j][0] ,edgecolors=colors_plt[j],marker='o'           , s=16,facecolors='none')
        elif errorbars == True:
            #https://matplotlib.org/api/_as_gen/matplotlib.axes.Axes.errorbar.html?highlight=matplotlib%20axes%20axes%20errorbar#matplotlib.axes.Axes.errorbar
            plt.errorbar(Redshift_bin[j][0:], Var2[j][0:], xerr = [E_Z1[j][0:],E_Z2[j][0:]], yerr = [E_V1[j][0:],E_V2[j][0:]],
                            fmt=markers_plt[j],color=colors_plt[j],elinewidth = 0.5, capsize = 0.5,
                            markeredgecolor=colors_plt[j], markerfacecolor = None,markeredgewidth=0.5,ms=14)

            plt.errorbar(Redshift_bin[j][1:], Var2[j][1:], xerr = [E_Z1[j][1:],E_Z2[j][1:]], yerr = [E_V1[j][1:],E_V2[j][1:]],
                            fmt=markers_plt[j],color=colors_plt[j], label=labels_plt[j], elinewidth = 1.5, capsize = 1.5,ms=14)

            plt.scatter(Redshift_bin[j][0], Var2[j][0]  ,marker='o',linewidths=1.5,edgecolors=colors_plt[j],s=350,facecolors='none')
            Redshift_bin[j][0] = Redshift_bin[j][0] - 0.01

    if plt_lit_res == True:
        var_lit_res = Table_Read_Lit(plt_lit_tbl,tbl_format_ipt)
        plt.scatter(x,y,color='black', marker = '*', s=10,label='Davis+14')
        XX = np.arange(-3,2,0.001)
        YY = 1.84*XX+10.80
        plt.scatter(XX,YY,color='gray',marker='.',s=1)
        print
        print
    elif plt_lit_res == False:
        pass
...
def MCMC_Confidence_Interval(iterations_mc,line1,line2,method,error,z_1,v_1,z_2,v_2,var_smpls_mcmc,nmb_smpls_mcmc,z_lne1_mcmc,z_lne2_mcmc,v_lne1_mcmc,v_lne2_mcmc,spec_file_plt_lne1_avg_mcmc,spec_file_plt_lne1_med_mcmc,spec_file_plt_lne2_avg_mcmc,spec_file_plt_lne2_med_mcmc,flx_lne1_avg_hdr,flx_lne1_med_hdr,flx_lne1_avg_hdr_e,flx_lne1_med_hdr_e,flx_lne2_avg_hdr,flx_lne2_med_hdr,flx_lne2_avg_hdr_e,flx_lne2_med_hdr_e,*args, **kwargs):
    ...
    p_z_lne1_mc  = n_z_lne1[index_z_lne1]     #pro
    z_z_lne1_mc  = bins_z_lne1[index_z_lne1]  #z

    i_z_lne2_mc  = index_z_lne2          #
    p_z_lne2_mc  = n_z_lne2[index_z_lne2]     #pro
    z_z_lne2_mc  = bins_z_lne2[index_z_lne2]  #z


    i_v_lne2_mc  = index_v_lne2          #
    p_v_lne1_mc  = n_z_lne1[index_v_lne2]     #pro
    v_z_lne1_mc  = bins_v_lne2[index_v_lne2]  #variable

    i_v_lne2_mc  = index_v_lne2          #
    p_v_lne2_mc  = n_z_lne2[index_v_lne2]     #pro
    v_z_lne2_mc  = bins_v_lne2[index_v_lne2]  #variable
    ...
    if plot_dist_hist == True:
        ...
        edgecolor='k')
        plt.subplots_adjust(
        left    = (56/25.4)/fxsize,         #22-def --> 26 bigger
        ...
        n_z_lne1, bins_z_lne1, patches_z_lne1 = ax110.hist(z_1, n_bins, density=1, histtype='step',cumulative=True,
                                        color='blue',alpha=0.7,label=label1_plt + ' Observed')
        n_z_lne2, bins_z_lne2, patches_z_lne2 = ax110.hist(z_2, n_bins, density=1, histtype='step',cumulative=True,
        ...
        n_z_lne1, bins_z_lne1, patches_z_lne1 = ax130.hist(z_z_lne1_mc, n_bins, density=False, histtype='step',cumulative=True,
                                        color='blue',alpha=0.7,label=label1_plt + ' MCMC')
        n_z_lne2, bins_z_lne2, patches_z_lne2 = ax130.hist(z_z_lne2_mc, n_bins, density=False, histtype='step',cumulative=True,
        ...
        n_z_lne1, bins_z_lne1, patches_z_lne1 = ax140.hist(v_z_lne1_mc, n_bins, density=False, histtype='step',cumulative=True,
                                        color='blue',alpha=0.7,label=label1_plt + ' MCMC')
        n_z_lne2, bins_z_lne2, patches_z_lne2 = ax140.hist(v_z_lne2_mc, n_bins, density=False, histtype='step',cumulative=True,
                                        color='red',alpha=0.7,label=label2_plt + ' MCMC')
        ...
        n_z_lne1, bins_z_lne1, patches_z_lne1 = ax150.hist(s_lne1_avg_mc, n_bins, density=False, histtype='step',cumulative=False,
                                        color='blue',alpha=0.7,label=label1_plt+' avg',linewidth=0.5)
        n_z_lne2, bins_z_lne2, patches_z_lne2 = ax150.hist(s_lne1_med_mc, n_bins, density=False, histtype='step',cumulative=False,
                                        color='black',alpha=0.7,label=label1 +' med',ls=':',linewidth=0.5)
        n_v_lne1, bins_v_lne1, patches_v_lne1 = ax150.hist(s_lne2_avg_mc, n_bins, density=False, histtype='step',cumulative=False,
                                        color='red',alpha=0.7,label=label2_plt+' avg',linewidth=0.5)
        n_v_lne2, bins_v_lne2, patches_v_lne2 = ax150.hist(s_lne2_med_mc, n_bins, density=False, histtype='step',cumulative=False,
        ...
        n_z_lne1, bins_z_lne1, patches_z_lne1 = ax160.hist(Lum_lne1_avg_mc_2[~np.isnan(Lum_lne1_avg_mc_2)], n_bins, density=False, histtype='step',cumulative=False,
                                        color='blue',alpha=0.7,label=label1_plt +' avg',linewidth=0.5)
        n_z_lne2, bins_z_lne2, patches_z_lne2 = ax160.hist(Lum_lne1_med_mc_2[~np.isnan(Lum_lne1_med_mc_2)], n_bins, density=False, histtype='step',cumulative=False,
                                        color='black',alpha=0.7,label=label1_plt + ' med',ls=':',linewidth=0.5)
        n_v_lne1, bins_v_lne1, patches_v_lne1 = ax160.hist(Lum_lne2_avg_mc_2[~np.isnan(Lum_lne2_avg_mc_2)], n_bins, density=False, histtype='step',cumulative=False,
                                        color='red',alpha=0.7,label=label2 +' avg',linewidth=0.5)
        n_v_lne2, bins_v_lne2, patches_v_lne2 = ax160.hist(Lum_lne2_med_mc_2[~np.isnan(Lum_lne2_med_mc_2)], n_bins, density=False, histtype='step',cumulative=False,
                                        color='black',alpha=0.7,label=label2_plt + ' med',ls='-.',linewidth=0.5)
        lg=plt.legend(loc=1,prop={'size':12})
        lg.draw_frame(False)
        ##########################################L MC DISTRIBUTION 1,2########################################
        ...
        n_z_lne1, bins_z_lne1, patches_z_lne1 = ax180.hist(Lum_lne1_lne2_avg_mc_1_rto[~np.isnan(Lum_lne1_lne2_avg_mc_1_rto)],
                                        n_bins, density=False, histtype='step',cumulative=False,
                                        color='blue',alpha=0.7,label=label1_plt+'/' +label2+' avg',linewidth=0.5)
...
def MCMC_generator(iterations_mc,line1,line2,method,error,sbsms_mcmc,sbsmn_mcmc,*args,**kwargs):
    ...
    func2           = kwargs.get('func2','med')
    plot_dist_hist  = kwargs.get('plot_dist_hist',True)
    spc_wdt_dir     = kwargs.get('spc_wdt_dir'    ,20)
    mask_radi_as_ms = kwargs.get('mask_radi_as_ms',15)

    immoments       = kwargs.get('immoments',True)
    continuum       = kwargs.get('continuum',True)
    ...
    if method == 1 and log_lm == None:
        ...
        flx_hdr     = 'C2G_FT1'#'C2G_FT1'
        lum_hdr     = 'C2G_LM1'#'C2G_LMT'
        flx_hdr_e   = 'C2G_F1E'#'C2G_F1E'
        lum_hdr_e1  = 'C2L_1'+str(error)+'L'#'C2L_T'+str(error)+'L'
        lum_hdr_e2  = 'C2L_1'+str(error)+'H'#'C2L_T'+str(error)+'H'

        lum_hdr_e11 = 'C2L_11L'#'C2L_T1L'
        lum_hdr_e21 = 'C2L_11H'#'C2L_T1H'
        lum_hdr_e12 = 'C2L_12L'#'C2L_T2L'
        lum_hdr_e22 = 'C2L_12H'#'C2L_T2H'
        lum_hdr_e13 = 'C2L_13L'#'C2L_T3L'
        lum_hdr_e23 = 'C2L_13H'#'C2L_T3H'

    elif method == 1 and log_lm == 'error':
        flx_hdr     = 'C2G_FT1'#'C2G_FT1'
        lum_hdr     = 'C2G_LM1'#'C2G_LMT'
        flx_hdr_e   = 'C2G_F1E'#'C2G_F1E'
        lum_hdr_e1  = 'C2M_1'+str(error)+'L'#'C2M_T'+str(error)+'L'
        lum_hdr_e2  = 'C2M_1'+str(error)+'H'#'C2M_T'+str(error)+'H'

        lum_hdr_e11 = 'C2M_11L'#'C2M_T1L'
        lum_hdr_e21 = 'C2M_11H'#'C2M_T1H'
        lum_hdr_e12 = 'C2M_12L'#'C2M_T2L'
        lum_hdr_e22 = 'C2M_12H'#'C2M_T2H'
        lum_hdr_e13 = 'C2M_13L'#'C2M_T3L'
        lum_hdr_e23 = 'C2M_13H'#'C2M_T3H'

    elif method == 1 and log_lm == 'value':
        flx_hdr     = 'C2G_FT1'#'C2G_FT1'
        lum_hdr     = 'C2G_LL1'#'C2G_LLT'
        flx_hdr_e   = 'C2G_F1E'#'C2G_F1E'
        lum_hdr_e1  = 'C2L_1'+str(error)+'L'#'C2L_T'+str(error)+'L'
        lum_hdr_e2  = 'C2L_1'+str(error)+'H'#'C2L_T'+str(error)+'H'
        ...
        flx_hdr     = 'C2G_FT2'#'C2G_FT2'
        lum_hdr     = 'C2G_LM2'#'C2G_LMF'
        flx_hdr_e   = 'C2G_F2E'#'C2G_F2E'
        lum_hdr_e1  = 'C2L_2'+str(error)+'L'# 'C2L_F'+str(error)+'L'
        lum_hdr_e2  = 'C2L_2'+str(error)+'H'# 'C2L_F'+str(error)+'H'

        lum_hdr_e11 = 'C2M_21L'#'C2L_F1L'
        lum_hdr_e21 = 'C2M_21H'#'C2L_F1H'
        lum_hdr_e12 = 'C2M_22L'#'C2L_F2L'
        lum_hdr_e22 = 'C2M_22H'#'C2L_F2H'
        lum_hdr_e13 = 'C2M_23L'#'C2L_F3L'
        lum_hdr_e23 = 'C2M_23H'#'C2L_F3H'

    elif method == 2 and log_lm == 'error':
        flx_hdr     = 'C2G_FT2'#'C2G_FT2'
        lum_hdr     = 'C2G_LM2'#'C2G_LMF'
        flx_hdr_e   = 'C2G_F2E'#'C2G_F2E'
        lum_hdr_e1  = 'C2M_1'+str(error)+'L'#'C2M_F'+str(error)+'L'
        lum_hdr_e2  = 'C2M_1'+str(error)+'L'#'C2M_F'+str(error)+'H'

        lum_hdr_e11 = 'C2M_21L'#'C2M_F1L'
        lum_hdr_e21 = 'C2M_21H'#'C2M_F1H'
        lum_hdr_e12 = 'C2M_22L'#'C2M_F2L'
        lum_hdr_e22 = 'C2M_22H'#'C2M_F2H'
        lum_hdr_e13 = 'C2M_23L'#'C2M_F3L'
        lum_hdr_e23 = 'C2M_23H'#'C2M_F3H'

    elif method == 2 and log_lm == 'value':
        flx_hdr     = 'C2G_FT2'#'C2G_FT2'
        lum_hdr     = 'C2G_LL2'#'C2G_LLF'
        flx_hdr_e   = 'C2G_F2E'#'C2G_F2E'
        lum_hdr_e1  = 'C2L_2' + str(error) + 'L'# 'C2L_F'+str(error)+'L'
        lum_hdr_e2  = 'C2L_2' + str(error) + 'H'# 'C2L_F'+str(error)+'H'
        ...
        lum_hdr_e22 = 'C2M_22H'#'C2M_F2H'
        lum_hdr_e13 = 'C2M_23L'#'C2M_F3L'
        lum_hdr_e23 = 'C2M_23H'#'S2M_F3H'

    elif method == 2 and log_lm == 'both':
        flx_hdr     = 'C2G_FT2'#'C2G_FT2'
        lum_hdr     = 'C2G_LL2'#'C2G_LLF'
        flx_hdr_e   = 'C2G_F2E'#'C2G_F2E'
        lum_hdr_e1  = 'C2M_2' + str(error) + 'L'#'C2M_F'+str(error)+'L'
        lum_hdr_e2  = 'C2M_2' + str(error) + 'H'#'C2M_F'+str(error)+'H'
        ...
        #S2M_F1L  2DGF Vol Lum T X CbeWth 1DGF FWHM [log] SUM   #6
        #S2M_F1H  2DGF Vol Lum T X CbeWth 1DGF FWHM [log] SUM   #6
        flx_hdr     = 'C2G_FT3'#'C2G_FT3'
        lum_hdr     = 'C2G_LM3'#'C2G_LMF'
        flx_hdr_e   = 'C2G_F3E'#'C2G_F3E'
        lum_hdr_e1  = 'C2L_3'+str(error)+'L'# 'C2L_F'+str(error)+'L'
        lum_hdr_e2  = 'C2L_3'+str(error)+'H'# 'C2L_F'+str(error)+'H'

        lum_hdr_e11 = 'C2M_31L'#'C2L_F1L'
        lum_hdr_e21 = 'C2M_31H'#'C2L_F1H'
        lum_hdr_e12 = 'C2M_32L'#'C2L_F2L'
        lum_hdr_e22 = 'C2M_32H'#'C2L_F2H'
        lum_hdr_e13 = 'C2M_33L'#'C2L_F3L'
        lum_hdr_e23 = 'C2M_33H'#'C2L_F3H'
        ...
        lum_hdr_e11 = 'C2M_31L'#'C2M_F1L'
        lum_hdr_e21 = 'C2M_31H'#'C2M_F1H'
        lum_hdr_e12 = 'C2M_32L'#'C2M_F2L'
        lum_hdr_e22 = 'C2M_32H'#'C2M_F2H'
        lum_hdr_e13 = 'C2M_33L'#'C2M_F3L'
        lum_hdr_e23 = 'C2M_33H'#'C2M_F3H'

    elif method == 3 and log_lm == 'value':
        flx_hdr     = 'C2G_FT3'#'C2G_FT3'
        lum_hdr     = 'C2G_LL3'#'C2G_LLF'
        flx_hdr_e   = 'C2G_F3E'#'C2G_F3E'
        lum_hdr_e1  = 'C2L_3' + str(error) + 'L'# 'C2L_F'+str(error)+'L'
        lum_hdr_e2  = 'C2L_3' + str(error) + 'H'# 'C2L_F'+str(error)+'H'
        ...
        lum_hdr_e22 = 'C2M_32H'#'C2M_F2H'
        lum_hdr_e13 = 'C2M_33L'#'C2M_F3L'
        lum_hdr_e23 = 'C2M_33H'#'S2M_F3H'

    elif method == 3 and log_lm == 'both':
        flx_hdr     = 'C2G_FT3'#'C2G_FT3'
        lum_hdr     = 'C2G_LL3'#'C2G_LLF'
        flx_hdr_e   = 'C2G_F3E'#'C2G_F3E'
        lum_hdr_e1  = 'C2M_3' + str(error) + 'L'#'C2M_F'+str(error)+'L'
        lum_hdr_e2  = 'C2M_3' + str(error) + 'H'#'C2M_F'+str(error)+'H'
    ...
    for element in sbsmn_mcmc:
        ...
        if immoments == True and continuum == False:
            clp_sfx = '.IM0'
            stk_sfx = str(spc_wdt_dir) + 'kms.IM0'
        ###CORRECTION FOR CONT####
        elif immoments == True and continuum == True:
            clp_sfx = '.cont'
            stk_sfx = str(spc_wdt_dir) + 'kms.IM0.CNT'
        else:
            pass
            clp_sfx = ''
            stk_sfx = str(spc_wdt_dir) + 'kms'

        prefix_line1    = line1 + '-'
        prefix_line2    = line2 + '-'
        stk_hme_dir_1   = home + 'Stack_Results-'+ line1 +'-2D/'
        ...
        stk_hme_dir_2   = home + 'Stack_Results-'+ line2 +'-2D/'
        ...
        spec_file_plt_red_1_avg = stp_dir_res_1 + prefix_line1 + 'CII_HATLAS-' + str(sbsms_mcmc) + '-' +str(element) +'-stk-'+func1 + '-'+stk_sfx+'-crc-'+str(mask_radi_as_ms)+'as_msk_ms.fits'
        spec_file_plt_red_1_med = stp_dir_res_1 + prefix_line1 + 'CII_HATLAS-' + str(sbsms_mcmc) + '-' +str(element) +'-stk-'+func2 + '-'+stk_sfx+'-crc-'+str(mask_radi_as_ms)+'as_msk_ms.fits'
        
        spec_file_plt_red_2_avg = stp_dir_res_2 + prefix_line2 + 'CII_HATLAS-' + str(sbsms_mcmc) + '-' +str(element) +'-stk-'+func1 + '-'+stk_sfx+'-crc-'+str(mask_radi_as_ms)+'as_msk_ms.fits'
        spec_file_plt_red_2_med = stp_dir_res_2 + prefix_line2 + 'CII_HATLAS-' + str(sbsms_mcmc) + '-' +str(element) +'-stk-'+func2 + '-'+stk_sfx+'-crc-'+str(mask_radi_as_ms)+'as_msk_ms.fits'
        ...
        TABLESTATNAME_11_PLT  = plt_dir_tbl_1 + '/CII_HATLAS-' + line1 + '-' + line2+'-' +str(sbsms_mcmc) + '-MS-' +str(method) + '-MC-'+str(iterations_mc)+'-' +str(method) +'-FLX-'+str(sbsmn_mcmc[0])+'-'+str(sbsmn_mcmc[-1])+'-stk-'+str(spc_wdt_dir)+'kms-crc-'+str(mask_radi_as_ms)+'as_msk_ms-stt-PLT.dat'
        TABLESTATNAME_12_PLT  = plt_dir_tbl_1 + '/CII_HATLAS-' + line1 + '-' + line2+'-' +str(sbsms_mcmc) + '-MS-' +str(method) + '-MC-'+str(iterations_mc)+'-' +str(method) +'-FLX-'+str(sbsmn_mcmc[0])+'-'+str(sbsmn_mcmc[-1])+'-stk-'+str(spc_wdt_dir)+'kms-crc-'+str(mask_radi_as_ms)+'as_msk_ms-stt-PLT.csv'
        TABLESTATNAME_21_PLT  = plt_dir_tbl_2 + '/CII_HATLAS-' + line1 + '-' + line2+'-' +str(sbsms_mcmc) + '-MS-' +str(method) + '-MC-'+str(iterations_mc)+'-' +str(method) +'-FLX-'+str(sbsmn_mcmc[0])+'-'+str(sbsmn_mcmc[-1])+'-stk-'+str(spc_wdt_dir)+'kms-crc-'+str(mask_radi_as_ms)+'as_msk_ms-stt-PLT.dat'
        TABLESTATNAME_22_PLT  = plt_dir_tbl_2 + '/CII_HATLAS-' + line1 + '-' + line2+'-' +str(sbsms_mcmc) + '-MS-' +str(method) + '-MC-'+str(iterations_mc)+'-' +str(method) +'-FLX-'+str(sbsmn_mcmc[0])+'-'+str(sbsmn_mcmc[-1])+'-stk-'+str(spc_wdt_dir)+'kms-crc-'+str(mask_radi_as_ms)+'as_msk_ms-stt-PLT.csv'
...
def Plot_CCube_Slices(Cube1,Cube2,Cube3,Cube4,Cube5,Cube6,*args, **kwargs):
    mask_in        = np.asarray((apfts.open(Cube1)[0]).data).squeeze()
    cub_crc_in     = np.asarray((apfts.open(Cube2)[0]).data).squeeze()
    cub_crc_wgt_in = np.asarray((apfts.open(Cube3)[0]).data).squeeze()
    mask_ot        = np.asarray((apfts.open(Cube4)[0]).data).squeeze()
    cub_crc_ot     = np.asarray((apfts.open(Cube5)[0]).data).squeeze()
    cub_crc_wgt_ot = np.asarray((apfts.open(Cube6)[0]).data).squeeze()
    ...
    PLOTFILENAME = plt_dir_res + prefix + (Cube1.split('.fits',1)[0]).rsplit('/',1)[-1] + '-slices.pdf'
    ...
    PLOTFILENAME = plt_dir_res + (Cube1.split('.fits',1)[0]).rsplit('/',1)[-1] + '-slices.pdf'
    fysize=11*2
    fxsize=8*6
    f = plt.figure(num=None, figsize=(fysize, fxsize), dpi=180, facecolor='w',
        edgecolor='k')
    ...
    plt.subplots_adjust(
        ...
        right   = 1 - (18/25.4)/fxsize,     
        top     = 1 - (15/25.4)/fysize)     

    circle=[]
    gs0 = gridspec.GridSpec(1, 1)
    gs11 = gridspec.GridSpecFromSubplotSpec(2, 3, subplot_spec=gs0[0],hspace=0.00,wspace=0.00)
    plt.suptitle(str(X_C)+','+str(Y_C) +  ' ' + '$\\varnothing_{\mathrm{in}}$: ' + str(radii_as_ot) + 'as ' + '$\\varnothing_{\mathrm{out}}$: ' + str(radii_as_in) + 'as  ',
                fontsize =20)
    ############################ UNO ##################################
    ax110 = plt.Subplot(f, gs11[0,0])
    ...
    plt.tick_params(which='both', width=1.0)
    plt.tick_params(which='major', length=10,labelsize=15)
    plt.tick_params(which='minor', length=5)
    ax110.minorticks_on()
    ...
    ax120 = plt.Subplot(f, gs11[0,1],sharey=ax110)
    ...
    ############################ TRES ##################################
    ax130 = plt.Subplot(f, gs11[0,2],sharey=ax120)
    ...
    ############################ UNO ##################################
    ax210 = plt.Subplot(f, gs11[1,0],sharex=ax110)
    ...
    ax220 = plt.Subplot(f, gs11[1,1],sharey=ax210,sharex=ax120)
    ...
    ax220.xaxis.set_tick_params(which='both',labelsize=4,direction='in',color='black',bottom=True,top=True,left=True,right=True)
    ax220.yaxis.set_tick_params(which='both',labelsize=4,direction='in',color='black',bottom=True,top=True,left=True,right=True)
    #ax220.set_title(str(slice_number+1))
    yticklabels = ax220.get_yticklabels()
    xticklabels = ax220.get_xticklabels()
    plt.setp(yticklabels, visible=False)
    plt.setp(xticklabels, visible=True)     
    ...
    ############################ TRES ##################################
    ax230 = plt.Subplot(f, gs11[1,2],sharey=ax220,sharex=ax130)
